...
class ImmutableAdjacencyMapUndirectedGraphInstance[F[_]: Monad]
    extends Graph[AdjacencyMap, F] {

  def edges[V, E](fg: F[AdjacencyMap[V, E]])(v: V): F[Iterable[E]] = fg.map {
    g => g.getOrElse(v, Map.empty).values
  }

  override def neighbours[V, E](fg: F[AdjacencyMap[V, E]])(
      v: V
  ): F[Iterable[(V, E)]] = fg.map(_.getOrElse(v, Map.empty))
  ...
  override def edgesTriples[V, E](
    ...
    fg.map(_.flatMap {
      case (v, edges) =>
        edges
          .map { case (n, e) => Set(v, n) -> e }
          .toMap
          .map {
            case (vs, e) if vs.size == 2 => (vs.head, e, vs.last)
            case (vs, e)                 => (vs.head, e, vs.head)
          }
    })
