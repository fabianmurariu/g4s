...
object GrBMatrixOps {
  ...
  def force[F[_]: Functor](p: F[MatrixPointer]) =
    p.map(mp => GrBError.check(GRBCORE.grbWaitMatrix(mp.ref)))
  ...
  def set[F[_]: Functor, A: SMH: ClassTag](
      pointer: F[MatrixPointer],
      is: Iterable[Long],
      js: Iterable[Long],
      vs: Iterable[A]
  ...
  def extract[F[_]: Monad, A: SMH: ClassTag](
      pointer: F[MatrixPointer]
  ...
  def release[F[_]: Functor](pointer: F[MatrixPointer]): F[Unit] =
    pointer.map(mp => GrBError.check(GRBCORE.freeMatrix(mp.ref)))

  
  def reduce0[F[_]:Sync, A:SVH](pointer: F[MatrixPointer],
