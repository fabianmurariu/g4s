    ...
    property(
      ...
      forAll { mt: MatrixTuples[A] =>
        ...
        val io = rM1M2
          .use {
            case (mat1, mat2, into) =>
              for {
                ...
                intersection <- ElemWise[IO].intersect(into)(Left(OP.plus))(
                  mat1,
                  mat2
                )
          ...
          .map {
            case (actualIntersect, actualUnion, areEqual) =>
              assert(
                areEqual,
                "intersect/union matrix with itself should yield the same matrix"
              )
              val expected =
                mt.tuples.map(t => N.times(t._3, N.fromInt(2))).sorted
              assertEquals(actualIntersect.sorted.toVector, expected)

              assertEquals(actualUnion.sorted.toVector, expected)
          }
