...
sealed trait OptimiserTask extends (() => Unit) with LazyLogging {
...
trait HasGroupExpression extends OptimiserTask {
  ...
  override def perform(): Unit = {
    logger.debug(
      s"${expr.groupId} - ${expr.childGroups} - ${expr.node} - ${expr.bestCost}"
    )
    apply()
  }
}
...
class OptimiseExpression(
  ...
  override def apply(): Unit = {
    ...
    val all = transform ++ implement //implement ++ transform
...
class ApplyRule(
  ...
  override def perform(): Unit = {
    if (!expr.hasRuleExplored(rule)) {
      logger.debug(s"${rule.getClass.getSimpleName} on ${expr}")
      apply()
    }
  }
